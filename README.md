...
- **Settings → Reader → Code Font** selects an installed monospaced font. **Programming Ligatures** offers Font Default, On and Off; shapes depend on the font. The same setting applies to the main Reader, comparison, Context and Reading Set. Copying always preserves source characters. Missing fonts fall back to the system font while retaining your choice; use **Refresh Fonts** after installing a font. Select **System Monospaced + Font Default** to restore the font policy. Files over 8,000 lines, or anchors on source lines over 64 KiB, preserve selection and horizontal position but skip synchronous precise viewport restoration. [Verified fonts and limitations](docs/plans/2026-09-22-reader-ligatures-acceptance.md).
