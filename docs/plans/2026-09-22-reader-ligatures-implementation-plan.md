...
- [x] 对 `!=` 中的 `=` 和 `!==` 中的子范围执行鼠标、键盘、查找与复制测试。见[解锁后桌面复验](evidence/reader-ligatures/desktop-20260923.md)。
...
- [x] 加入辅助功能标签和键盘操作验收。AX 控件自动化及真实 Settings 菜单键盘操作已执行，见[桌面记录](evidence/reader-ligatures/desktop-20260923.md)。
...
2026-09-23 按用户明确决定：33 ms 等未经实际数据或理论支持的性能数值只作 best-effort 参考，不作为硬性验收阻塞；其他类似性能指标同样处理。保留测量口径、原参考值和真实结果，不改写历史数据。功能正确性、资源有界性、必需用例与完整 CI 仍按原要求执行。
...
- [x] 运行既有 fold、projector、wrap 及产品质量门槛，参数按当前脚本契约执行。（此前完整产品门禁重跑通过；最终代码的完整回归结果另见验收记录。F1 等性能参考值不再作为发布阻塞。）
- [x] 测量默认配置相对原基线，以及同字体 On 相对 Off 的回归。默认基线见[对照记录](evidence/reader-ligatures/default-baseline/comparison.json)，各版本实测见验收记录。
...
- [x] 记录 feature 请求表、缓存策略和性能参考目标及实测。
- [x] 已将文档状态更新为“已实施”；最终源码`0296f21`，完整1033项测试、18通道及gold gates通过。

**P5 门槛：** 所有必需 A 项的功能与正确性通过，性能实测与基线差异完整披露；完整 CI 有成功摘要；没有被字体缺失或环境跳过掩盖的必需用例。未经证据支持的性能参考值不单独决定通过或失败。
...
逐项现状见[验收记录](2026-09-22-reader-ligatures-acceptance.md)。下方保留记录格式。原生鼠标与菜单修复已完成[可见桌面复验](evidence/reader-ligatures/desktop-20260923.md)；最终代码的全量回归仍以验收记录中的完整结果为准。
...
性能记录同时保存参考目标版本、样本数和测试脚本参数。更新参考目标须说明改变原因及影响，不用新目标覆盖原来的结果；超出 best-effort 参考值如实披露，不伪装成达标，也不单独判定验收失败。
