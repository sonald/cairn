...
- [ ] 启动后看到三面板空态窗口，无项目自动恢复或磁盘扫描。
- [ ] 菜单骨架可见，`Cmd+Q` 正常退出。

### S2 — 打开项目、文件树、引擎生命周期

- [ ] `Cmd+O` 打开 tokio，文件树 1s 内可见且只列 Rust 路径。
- [ ] `Indexing 717 files…` 出现后消失；索引期间窗口、文件树均可交互。

### S3 — 阅读区产品化

- [ ] 打开 tokio 常规文件，首屏即时出现且高亮、字号层级、1.25 行距正确。
- [ ] 打开 10 万行文件，先见纯文本、后补高亮；滚动无明显 hitch，最大文件可滚到底。
- [ ] 复制到纯文本目标不携带样式；切换系统深浅色后阅读区配色正确。

### S4 — 符号模糊搜索

- [ ] `Cmd+T` 输入 `jhndl`，`JoinHandle` 位于前排。
- [ ] Enter 跳到选中符号，Esc 关闭；快速连续输入时列表不闪烁或倒退。

### S5 — Context Window

- [ ] 单击引用后 50ms 内出现 excerpt，并显示 certainty、dispatch 标签。
- [ ] 单击局部变量时显示声明与 binding kind；Pin 后普通点击不替换内容。
- [ ] `‹` / `›` 可循环候选；`Cmd+单击` 跳转并同步 Context；仅 hover 不触发查询。

### S6 — 跳转历史

- [ ] 完成 A→B→C 后连续后退两次回到 A 的原视口，再前进恢复 B、C。
- [ ] Go 菜单与 `Cmd+Ctrl+←/→`、`Cmd+[/]` 可用；到达两端时菜单正确灰显。

### S7 — 性能验收与收尾

- [ ] 按下方脚本连续阅读 tokio 30 分钟，期间无崩溃、卡死或错误跳转。
- [ ] 每次产生“想切回 IDE”的冲动时，记录时间、正在做的动作和缺失能力，作为 M2 输入。

### 30 分钟 tokio 走查脚本

- [ ] **0–3 分钟：启动与项目。** release App 冷启动；`Cmd+O` 打开 tokio，确认文件树先出现、索引状态随后结束。
- [ ] **3–8 分钟：从 spawn 入手。** `Cmd+T` 搜索 `spawn`，打开 `tokio::spawn` 定义，阅读周边实现并上下滚动。
- [ ] **8–15 分钟：Context 逐层。** 依次单击参数、局部变量、直接调用和 trait/method 调用；查看候选标签、excerpt，使用 `‹` / `›`，再 Pin 一个结果。
- [ ] **15–20 分钟：跨文件跳转。** 从 Context 与 `Cmd+单击` 进入至少三个不同文件，确认目标位置与 Context 同步。
- [ ] **20–24 分钟：历史往返。** 对刚才的 A→B→C 路径后退到 A、再前进到 C，核对每次恢复的文件与视口。
- [ ] **24–28 分钟：最大文件。** 在文件树打开 tokio 最大 `.rs` 文件，立即阅读首屏，持续滚到文件底部并观察异步高亮过程。
- [ ] **28–30 分钟：回顾。** 再做一次符号搜索和 Context 跳转；汇总“想切回 IDE”记录，标出第一个阻断阅读流的点。
