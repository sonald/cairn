...
@main struct Probe {
    @MainActor static func main() throws {
        ...
        let end = content.location(target, offsetBy: 1)!
        manager.ensureLayout(for: NSTextRange(location: target, end: end)!)
        mark("anchorLayout")
        ...
        mark("tile")
        if let relocated {
            scroll.contentView.setBoundsOrigin(NSPoint(x:0,y:relocated))
            mark("clipScroll")
            scroll.reflectScrolledClipView(scroll.contentView)
            mark("reflect")
        }
