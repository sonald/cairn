...
@main struct Probe {
    @MainActor static func main() throws {
        ...
        if !usage.isEmpty, !usage.isNull {
            view.setFrameSize(NSSize(width: max(scroll.contentSize.width,usage.maxX+20), height:max(scroll.contentSize.height,usage.maxY+24)))
        }
        scroll.tile()
        if let relocated { scroll.contentView.scroll(to:NSPoint(x:0,y:relocated)); scroll.reflectScrolledClipView(scroll.contentView) }
        mark("resizeScroll")
