...
各阶段的规格、自动化和 UI/AX 证据分别见 `checkpoint-a.md`、`checkpoint-b.md`、`checkpoint-c.md`、`reading-set-prototype.md` 与三张 `resolution-inspector-*.png`。

## V0 自动化门禁

| 门禁 | 结论 | 证据 |
|---|---|---|
| `swift test --disable-sandbox` | PASS | 433 / 433 |
| `scripts/provision-corpora.sh --check` | PASS | tokio 720 个 `.rs`，`be8ee45`；ripgrep 98 个 `.rs`，`4649aa9` |
| `CODEX_SANDBOX=1 bash scripts/ci.sh` | PASS | build、433 tests、12 self-tests、protected-path checks 全通过 |
| `scripts/run-self-tests.sh` | PASS | 12 / 12；artifact `.build/self-test-run-20260807-124705-23070` |
| `scripts/stress-test.sh --runs 5 --load 8 --timeout 180` | PASS | 5 / 5；0 failure、0 hang、0 error；61 / 45 / 50 / 53 / 44 秒；artifact `.build/stress-test-20260807-125036-27259`；residual process 0 |
...
所有最终截图均来自独立 bundle id `dev.cairn.CairnM10` 的临时验收包，实际路径 `.build/m10-v0-unique/Cairn.app`，避免 Computer Use 自动唤起旧 `.build/distribution/Cairn.app`。该 bundle id 只用于验收，没有进入产品源码或提交。
...
PASS。原生 AppKit 最终实现与设计合同一致：Relations / Inspector 等宽且不重叠；三值 badge、位置维度 chip、corrected disclosure、SOURCE / VERIFICATION / AVAILABILITY / ENVIRONMENT 分段及 `Show full audit` 渐进披露均有几何、交互和 AX 断言。light、dark、si-classic 三张 1600×1000 截图经人工逐项审计；没有增加 SwiftUI bridge、第二状态源或非文件 tab。
...
- PASS：P0 仅落文档，未进入 `Sources/`；未实现 M11 Reading Set、非文件 tab、StableSymbolID 或额外抽象。
...
PASS。逐条回读 §1、§3、E0–E3、N1–N2、P0、V0 后未发现剩余 FAIL；分层无循环依赖，snapshot 不含 ID/ref，trace 不把 stale/cancelled 写入证据，call-site reconciliation、narrative、Inspector、Trail 与 snapshot replay 均有可运行检查。保留的实现均有当前需求，没有为 M11/M12 预建实体。
