...
public final class Materializer: @unchecked Sendable {
    ...
    private func materialize(
        ...
        if FileManager.default.fileExists(atPath: marker.path) {
            ...
            if retaining { retainLocked(destination) }
    ...
    private func enforceQuota(keeping current: URL) throws {
        ...
        // again once their references drop to zero (§8.3).
...
public enum ExactError: Error { case invalidPath(String) }
public struct FakeOID { public let hex = "commit" }
public enum FakeMode { case regular }
public struct FakeFile { public let path = "unused"; public let fileMode = FakeMode.regular }
public struct CommitSnapshot {
 public let commitOID = FakeOID()
 public func listFiles() -> [FakeFile] { [] }
 public func readBytes(path: String) throws -> [UInt8] { [] }
}

let root = URL(fileURLWithPath: "/tmp/cairn-materializer-independent/cache")
let fm = FileManager.default
try? fm.removeItem(at: root)
let inaccessible = root.appendingPathComponent("oldcommit")
try fm.createDirectory(at: inaccessible, withIntermediateDirectories: true)
try fm.setAttributes([.posixPermissions: 0], ofItemAtPath: inaccessible.path)
let m = Materializer(rootURL: root)
do {
 _ = try m.materializeAndRetain(CommitSnapshot(), configFingerprint: "config")
 print("unexpected success")
} catch { print("materialize failed:", error) }
try fm.setAttributes([.posixPermissions: 0o700], ofItemAtPath: inaccessible.path)
print("references after failed materialize:", m.retainedDirectoryCount)
do { try m.clear(); print("clear succeeded") } catch { print("clear failed:", error) }
try? fm.removeItem(at: root)
